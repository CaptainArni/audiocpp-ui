...
- **Telemetry** has a **Free VRAM** button (and a per-model one). Models load on first use and are then held indefinitely, which is right for latency and awkward on a box also running a large chat model; unloading is safe because the next request reloads transparently.
